...
#define JUMP_PIXEL 30 // Number of pixel dino will jump

#define JUMP_BUTTON_PIN 2  // Pin for the jump button
#define START_BUTTON_PIN 3 // Pin for the start button
...
void setup()
{
    ...
    Serial.begin(9600);
    pinMode(JUMP_BUTTON_PIN, INPUT_PULLUP); // Setup the jump button pin as input with internal pull-up resistor
    pinMode(START_BUTTON_PIN, INPUT_PULLUP); // Setup the start button pin as input with internal pull-up resistor
...
void play()
{
    ...
    for (;;)
    {
        ...
        if (digitalRead(JUMP_BUTTON_PIN) == LOW) // Check if the jump button is pressed
        {
            Serial.println("Jump");
            if (jump == 0)
                jump = 1;
